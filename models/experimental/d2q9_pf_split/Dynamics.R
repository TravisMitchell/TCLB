...
if (Options$RT) {
	...
	AddStage("BaseInit"  , "Init_distributions" , save=Fields$group %in% c("f","g","h","Vel"))
	
	# iteration
	AddStage("BaseIter"  , "calcHydroIter" 		, save=Fields$group %in% c("f","g","h","Vel")		, load=DensityAll$group %in% c("f","g","h","Vel"))
	...
	# iteration
	AddStage("BaseIter"  , "calcHydroIter"      , save=Fields$group %in% c("f","g","h","Vel","nw") , load=DensityAll$group %in% c("f","g","h","Vel","nw"))  # TODO: is nw needed here?
	AddStage("PhaseIter" , "calcPhaseFIter"		, save=Fields$group %in% c("PF")			   , load=DensityAll$group %in% c("f","g","h","Vel","nw"))
...
AddSetting(name="alpha",default="1",comment='splitting prm')
