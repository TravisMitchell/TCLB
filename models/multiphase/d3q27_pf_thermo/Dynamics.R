...
AddStage("FiniteDifference", "TempUpdate", save=Fields$group=="Thermal", load=DensityAll$group %in% c("Vel","nw","PF","Thermal"))
AddAction("TempToSteadyState", "FiniteDifference")
...
if (Options$OutFlow){
    ...
    AddStage("BaseIter"  , "Run"       ,         save=Fields$group %in% c("g","h","Vel","nw","Thermal"), 
	                                	 load=DensityAll$group %in% c("g","h","Vel","nw","Thermal"))
}
